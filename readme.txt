...
* `no_shortcode`, shortcodes are always stripped from excerpts to prevent memory exhaustion; this parameter is retained for backwards compatibility but no longer changes behaviour
...
== Changelog ==
= 4.5.1-fork =
* New - Homepage Category Filter (multi-select, shows posts from any selected category; also applies to the site's own default RSS/Atom feed, e.g. `/feed/`)
* New - Block Finish Mode: stops the excerpt at the next block-level tag, <br>, or the close of the current inline element (link, bold, etc.) instead of cutting mid-element or running on indefinitely through a stretch of inline content
* Fix - Smart Tag Closing: excerpts always end with balanced, valid HTML; consecutive <br> tags are collapsed to one, everywhere including RSS feeds (a single <br> is valid, meaningful markup that any HTML-capable reader renders fine)
* New - Anchor Tag Sanitization: only <a> tags with a real, followable href are kept when anchors are not stripped per settings — empty hrefs, same-page fragments, bare anchors with no href, and javascript:/data:/vbscript: pseudo-schemes are unwrapped to their visible text instead
* New - Header Content Skipping: an option to exclude text within H1-H6 from excerpt content
* New - RSS-Safe Output: RSS Max Length setting truncates feed output while keeping HTML valid and properly closed
* New - Slack-friendly HTML-to-text conversion for lists, blockquotes, definition lists, and horizontal rules; a <br> becomes a real line break; links use a minimal <a href="URL">Text</a> tag stripped down to nothing but href, with a literal "|" occurring naturally in link text replaced with a slash; converted bullet/numbered lists no longer inherit blank lines between items or around a nested sublist from Gutenberg's list markup, regardless of settings
* New - Strip Empty Lines from Slack Feeds (on by default): collapses blank lines between top-level blocks
* New - Strip Links from Slack Feeds (on by default): removes every link entirely, even if <a> is otherwise allowed by Strip Tags, and inserts an invisible character into any URL-shaped plain text strings within the feed, in order to prevent Slack from auto-linking them
* New - Advanced List, Table and Top-Level Structure Handling: max list items, max top-level list items/table rows, and max top-level structures options, with a separate List/Table Ellipsis marker for truncated lists and tables
* New - [excerpt_cut] and [excerpt_only] shortcodes to hide or show content specifically in excerpts vs. full posts
* Fix - Plain text shaped like a tag (e.g. "<Free>", "<New>") is no longer mistaken for real markup: it used to get silently swallowed and could leave an invalid closing tag auto-appended at the end
* Fix - OOM on archive/search pages: replaced apply_filters('the_content') with safe core formatting functions to prevent plugin hooks (do_shortcode, term queries, etc.) exhausting memory when generating multiple excerpts
* Note - Shortcodes are now always stripped from excerpt source text; [excerpt_cut], [excerpt_only], and [advanced_excerpt_text] continue to work as before

= 4.5 (August 1st, 2026) =
* Fix - Security issue (credit to Mustafa Ahmed)
